...
class MatchdayCommandTests(unittest.TestCase):
    ...
    def test_played_match_has_no_due_actions_left(self):
        """T-0176: Status 'gespielt' entkraeftet die Pre-Match-Checks -- dann
        duerfen sie auch nicht mehr als faellig gezaehlt oder gerendert werden.

        Rueckfall der T-0169-Klasse: dort wurde der `played`-Guard in
        `build_watchlist` nachgezogen, die Aktions-Pfade blieben aussen vor.
        Real gemessen am 23.08.26: `open_due` 104 bei `played` 104.
        """
        report = build_matchday_command_center(
            {"fixtures": [fixture()]},
            {"predictions": [prediction(critical=True)]},
            team_intel_payload(),
            {"checks": {}, "matches": {}},
            now=datetime(2099, 6, 11, 21, 0, tzinfo=timezone.utc),  # 2h nach Anpfiff
            write=False,
        )
        row = report["items"][0]
        self.assertEqual(row["status"], "gespielt")
        self.assertEqual(row["due_actions"], [], "gespielte Spiele haben keine faelligen Checks mehr")
        # next_actions gehoert zur selben Klasse: `append_rows` faellt darauf
        # zurueck, wenn due_actions leer ist -- ein halber Fix wuerde die Checks
        # nur eine Zeile weiter unten wieder anzeigen.
        self.assertEqual(row["next_actions"], [], "auch keine 'naechsten' Checks mehr")
        self.assertIsNone(row["next_action_at"])
        self.assertEqual(report["summary"]["open_due"], 0)
        self.assertEqual(report["next_items"], [])
        # Die Roh-Aktionen bleiben zur Nachvollziehbarkeit erhalten -- nur ihre
        # Faelligkeit ist erloschen.
        self.assertTrue(row["actions"], "Aktions-Historie bleibt sichtbar")
