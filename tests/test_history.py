...
class HistoryTests(unittest.TestCase):
    ...
    def test_foreign_round_recommendation_is_not_logged_as_default_round(self):
        """T-0192: `recommended_tip` faellt fuer Stages, die die Default-Runde
        nicht tippt, auf den Tipp einer ANDEREN Runde zurueck (Anzeige-Hilfe
        aus `model.predict_fixture`). Die History darf ihn dann nicht unter
        der Default-Runde fuehren -- sonst stuende dort ein Tipp mit fremder
        Rundenwertung unter falschem Rundennamen.
        """
        def fremd(tip):
            row = prediction(tip)
            row["round_tips"] = {
            ...
            row["recommended_tip"] = dict(row["round_tips"][SECONDARY_ROUND_ID])
            return row

        self.assertIsNone(prediction_change_event(fremd("1:0"), fremd("2:1")))
        # Ueber die Zweitrunde ist derselbe Wechsel weiterhin sichtbar.
        event = prediction_change_event(
            fremd("1:0"), fremd("2:1"), round_id=SECONDARY_ROUND_ID
        )
        self.assertIsNotNone(event)
        self.assertEqual(event["round_name"], ROUND_NAME_2)

    def test_legacy_recommendation_without_round_id_still_counts(self):
        """Aeltere Payloads fuehren kein `round_id` im Tipp -- die gelten
        weiter als Default-Runde, sonst verschwaende die bestehende History."""
        def alt(tip):
            row = prediction(tip)
            row["round_tips"] = {}
            row["recommended_tip"] = {"tip": tip, "expected_points": 2.0}
            return row

        event = prediction_change_event(alt("1:0"), alt("2:1"))
        self.assertIsNotNone(event)
        self.assertEqual(event["to_tip"], "2:1")
