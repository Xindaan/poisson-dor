...
from wm_tipps.round_rules import TipRound
from wm_tipps.scoring import Score, is_knockout_stage, kicktipp_points
...
class FinalsParserTests(unittest.TestCase):
    def test_parses_ko_matches_excluding_third_place(self):
        ...
        self.assertTrue(all(is_knockout_stage(r["stage"]) for r in rows))

    def test_ko_rows_carry_their_real_stage(self):
        """T-0187: jede KO-Zeile traegt die Stage ihrer Sektion, nicht 'knockout'."""
        rows = {r["match"]: r for r in parse_openfootball_finals(SAMPLE_FINALS)}
        self.assertEqual(rows["France - Argentina"]["stage"], "round_of_16")
        self.assertEqual(rows["Spain - Russia"]["stage"], "round_of_16")
        self.assertEqual(rows["France - Croatia"]["stage"], "final")

    def test_ko_stage_reaches_rounds_with_stage_escalation(self):
        """Der Grund, warum die echte Stage zaehlt: eine Runde, die ab dem
        Achtelfinale eskaliert, muss diese Punkte im Backtest auch bekommen.

        Mit dem alten pauschalen "knockout" fand `points_for_stage` keinen
        `stage_points`-Eintrag und fiel auf den Basissatz zurueck -- gemessen
        170 statt 340 Punkte ueber die 105 KO-Spiele der sieben Datensaetze.
        """
        eskalierend = TipRound(
            id="eskalierend",
            name="Eskalierend",
            group_points={"tendency": 2, "difference": 3, "exact": 4},
            knockout_points={"tendency": 2, "difference": 3, "exact": 4},
            stage_points={
                "round_of_16": {"tendency": 4, "difference": 6, "exact": 8},
                "final": {"tendency": 4, "difference": 6, "exact": 8},
            },
            bonus_questions=(),
            bonus_points={},
            result_scope="nach Verlaengerung",
        )
        rows = {r["match"]: r for r in parse_openfootball_finals(SAMPLE_FINALS)}
        with mock.patch.dict(scoring.TIP_ROUNDS, {"eskalierend": eskalierend}, clear=False):
            punkte = [
                kicktipp_points(
                    Score(*rows[match]["actual"]),
                    Score(*rows[match]["actual"]),
                    rows[match]["stage"],
                    round_id="eskalierend",
                )
                for match in ("France - Argentina", "France - Croatia")
            ]
        # Exakter Treffer in einer eskalierten Stage = 8, nicht der Basissatz 4.
        self.assertEqual(punkte, [8, 8])
