...
class BacktestEngineTests(unittest.TestCase):
    ...
    def test_knockout_matches_counts_real_stages(self):
        """T-0187: die Datensaetze tragen echte Stages (round_of_16, quarter,
        semi, final) statt des pauschalen "knockout" -- die KO-Zaehlung darf
        nicht auf das alte Literal hoeren, sonst meldet der Report 0 KO-Spiele.
        """
        rows = [
            {"match": "A - B", "stage": "round_of_16", "actual": [1, 0],
             "pre_elo": {"home": 1800, "away": 1700}},
            {"match": "C - D", "stage": "final", "actual": [2, 1],
             "pre_elo": {"home": 1800, "away": 1700}},
            {"match": "E - F", "stage": "group", "actual": [0, 0],
             "pre_elo": {"home": 1800, "away": 1700}},
        ]
        path = _write_dataset(rows)
        try:
            result = run_backtest(path, round_id=DEFAULT_ROUND_ID)
        finally:
            path.unlink()
        self.assertEqual(result["knockout_matches"], 2)
        self.assertEqual(result["group_matches"], 1)
