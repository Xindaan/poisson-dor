...
class ScoringAndActualTests(unittest.TestCase):
    ...
    def test_string_actual_is_counted_as_played(self):
        ...
        pbi = {"m1": {"fixture": {"stage": "group"}}, "m2": {"fixture": {"stage": "group"}}}
        prof = _profile("T", tips, actuals, {}, pbi, DEFAULT_ROUND_ID)
        self.assertEqual(prof["played"], 2)
        self.assertEqual(prof["exact_rate"], 0.5)  # m1 exakt (0:1)

    def test_missing_stage_is_not_scored_as_group(self):
        # T-0179-Regression: fehlt zu einem gespielten Match die Prediction
        # (und damit die Stage), darf es NICHT still mit Gruppen-Punkten
        # gezaehlt werden. ko1 waere bei Gruppen-Wertung 4 Punkte (exakt);
        # richtig ist: ungewertet, sichtbar in skipped_stage.
        tips = {"ko1": "2:0"}
        actuals = {"ko1": [2, 0]}
        prof = _profile("X", tips, actuals, {}, {}, DEFAULT_ROUND_ID)  # keine Prediction fuer ko1
        self.assertEqual(prof["played"], 0)
        self.assertEqual(prof["points"], 0)
        self.assertEqual(prof["skipped_stage"], 1)

    def test_stage_of_returns_none_when_unknown(self):
        self.assertIsNone(_stage_of({}, "ko1"))
        self.assertIsNone(_stage_of({"ko1": {"fixture": {}}}, "ko1"))
        self.assertEqual(_stage_of({"ko1": {"fixture": {"stage": "round_of_16"}}}, "ko1"), "round_of_16")
...
class RoundAndVerdictTests(unittest.TestCase):
    def test_round_sorts_by_points_and_computes_corr(self):
        ...
        # Predictions mit Stage noetig (T-0179): ohne Stage zaehlt kein Match mehr.
        predictions = [{"match_id": f"m{i}", "fixture": {"stage": "group"}} for i in range(10)]
        rd = _round_profiles(DEFAULT_ROUND_ID, players, actuals, predictions)
