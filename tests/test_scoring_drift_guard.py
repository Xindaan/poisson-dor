...
Die Kicktipp-Wertung ist VIERFACH implementiert: Kern (`wm_tipps.scoring`),
`wm_tipps.rival_profiles._pool_points` (T-0177 nachgetragen), das explorative
`analysis/rival_lab.py` und `analysis/rival-profiles/tidy.py`.
...
T-0177 (23.08.26): `rival_profiles._pool_points` fehlte hier -- eine produktive
Kopie im Kern-Paket, die der Guard nicht sah. `test_rival_profiles.py` pinnt sie
zwar, aber nur mit festen Zahlen fuer `group` und die Default-Runde plus einen
K.o.-Elferfall. Gemessene Luecke: ein Drift, der NUR `third_place` trifft, lief
durch die komplette Suite (629/629 gruen). Deshalb steht die Kopie jetzt im
Kreuzvergleich ueber alle Stages und beide Runden -- die Achsen, auf denen die
Regel schon dreimal nachgezogen werden musste (T-0097, T-0120, T-0130).
"""
...
from wm_tipps.rival_profiles import _pool_points  # noqa: E402
...
# Eine Runde, die es in keinem lokalen Literal gibt. Wer die Punktetabelle ueber
# Runden-IDs statt ueber die Rundenkonfiguration entscheidet, faellt hier auf
# Gruppenwertung zurueck -- und genau das soll rot werden (T-0178).
FREMDE_RUNDE = "guard-runde-mit-fremdem-namen"


def _mit_fremder_runde():
    ...
    fremd = TipRound(
        ...
        result_scope=vorlage.result_scope,
        stage_points=vorlage.stage_points,
    )
    return mock.patch.dict(scoring.TIP_ROUNDS, {FREMDE_RUNDE: fremd})
...
class ScoringDriftGuardTests(unittest.TestCase):
    ...
    def test_points_tables_folgen_der_rundenkonfiguration_nicht_dem_namen(self):
        """T-0178: die Tabelle kommt aus der Rundenkonfiguration, nicht aus einer
        lokalen Liste bekannter Runden-IDs.

        Ohne diesen Test bleibt ein Rueckfall auf die alte Literal-Tabelle in
        Andres Installation unsichtbar (die IDs passen dort zufaellig) und faellt
        erst unter den neutralen Defaults auf -- dort mit 12 falschen
        Stage/Runden-Kombinationen, gemessen am 23.08.26.
        """
        with _mit_fremder_runde():
            for stage in STAGES:
                core = dict(core_table(stage, FREMDE_RUNDE))
                with self.subTest(stage=stage):
                    self.assertEqual(
                        core, dict(_RIVAL_LAB.points_table(stage, FREMDE_RUNDE)),
                        "rival_lab.points_table entscheidet ueber den Rundennamen "
                        "statt ueber die Rundenkonfiguration.",
                    )
                    self.assertEqual(
                        core, dict(_TIDY.points_table(stage, FREMDE_RUNDE)),
                        "tidy.points_table entscheidet ueber den Rundennamen "
                        "statt ueber die Rundenkonfiguration.",
                    )
...
class PoolPointsDriftGuardTests(unittest.TestCase):
    """Vierte Kopie: `rival_profiles._pool_points` (Kern-Paket, stdlib).

    Eigene Klasse ohne `skipUnless`: dieser Zweig braucht den numpy/pandas-
    Carve-out nicht und muss deshalb auch in einer bare-stdlib-Umgebung laufen.
    """

    def test_pool_points_agrees_with_core(self):
        for round_id in ROUND_IDS:
            for stage in STAGES:
                for th, ta, ah, aa in SCENARIOS:
                    core = core_score(Score(th, ta), Score(ah, aa), stage, round_id)
                    pool = _pool_points((th, ta), (ah, aa), stage, round_id)
                    self.assertEqual(
                        core, pool,
                        "Scoring Drift Kern<>rival_profiles._pool_points @ "
                        f"{round_id}/{stage} tip {th}:{ta} act {ah}:{aa}",
                    )
