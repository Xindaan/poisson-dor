...
def _round_tips(prediction: Mapping[str, Any]) -> dict[str, Mapping[str, Any]]:
    ...
    if isinstance(recommended, Mapping) and recommended and DEFAULT_ROUND_ID not in tips:
        # T-0192: nur uebernehmen, wenn der empfohlene Tipp WIRKLICH der der
        # Default-Runde ist. `model.predict_fixture` faellt fuer Stages, die
        # die Default-Runde nicht tippt, auf den Tipp einer anderen Runde
        # zurueck (Anzeige-Fallback) -- den hier unter die Default-Runde zu
        # schieben, erzeugte einen History-Eintrag mit fremder Rundenwertung
        # unter falschem Rundennamen. Aeltere Payloads ohne `round_id` gelten
        # weiterhin als Default-Runde, sonst verschwaende die History.
        recommended_round = recommended.get("round_id")
        if recommended_round in (None, DEFAULT_ROUND_ID):
