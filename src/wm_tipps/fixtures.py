...
def _load_manual_results() -> dict[str, dict[str, Any]]:
    ...
    for match_id, value in (results or {}).items():
        if isinstance(value, list) and len(value) == 2:
            ...
            # T-0185: Laenge pruefen, nicht nur truthy -- sonst wirft
            # `{"actual": [2]}` IndexError, waehrend die aequivalente blanke
            # Liste `[2]` eine Zeile darueber still uebersprungen wird.
            # Gleiche Klasse wie in role_experiment.normalize_manual_results.
            # `shootout` wird hier bewusst NICHT durchgereicht: Fixture-Zeilen
            # tragen kein Elferfeld (verifiziert -- niemand liest es von dort),
            # die Wertung zieht es direkt aus manual_results.
...
def _with_knockout_fixtures(
    ...
    *,
    ...
    # T-0190: manual_results ist als Korrektur-Override dokumentiert, wirkte aber
    # nur auf K.o.-Zeilen. Ein falsches Gruppenergebnis in openfootball war damit
    # unkorrigierbar -- und Gruppentabellen entscheiden die K.o.-Slots, also haette
    # genau dort ein Override am meisten geholfen. Vor den Gruppentabellen
    # anwenden, damit die Korrektur bis in den Baum durchschlaegt.
    group_fixtures = _apply_manual_results(group_fixtures, manual_results)
