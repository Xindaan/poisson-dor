...
def _played_results() -> dict:
    # T-0188: eine Quelle statt einer eigenen Mischung. Die frueher hier
    # gebaute Variante liess `penalty_winner` und `shootout` ganz weg --
    # damit wertete signal-breaker/totals-adjust jedes K.o.-Remis falsch.
    from .results import played_results

    return played_results(load_fixture_payload().get("fixtures", []))
...
def cmd_backtest_report(args: argparse.Namespace) -> dict:
    from .backtest import report_paths_for_round
    from .scoring import DEFAULT_ROUND_ID, TIP_ROUNDS

    round_id = getattr(args, "round", None) or DEFAULT_ROUND_ID
    if round_id not in TIP_ROUNDS:
        raise SystemExit(
            f"Unbekannte Runde: {round_id}. Verfuegbar: {', '.join(TIP_ROUNDS)}"
        )
    json_path, markdown_path = report_paths_for_round(round_id)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    backtest.add_argument(
        "--tournament",
        choices=["sample", "2010", "2014", "2018", "2022", "euro-2016", "euro-2020", "euro-2024"],
        default="sample",
        help="sample (7-Spiele-Sample) oder ein echtes Turnier aus openfootball.",
    )
    ...
    backtest_report.add_argument(
        "--include-sample",
        action="store_true",
        help="Das 7-Spiele-Sample zusaetzlich zu den sieben Turnieren aufnehmen.",
    )
    backtest_report.add_argument(
        "--round",
        default=None,
        help=(
            "Rundenprofil fuer die Wertung (Default: die Hauptrunde). Jede "
            "weitere Runde schreibt eigene Report-Dateien."
        ),
    )
