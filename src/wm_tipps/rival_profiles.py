...
def _pool_points(pred, act, stage, round_id):
    """Echte Pool-Regel -- duenner Adapter auf den Kern, KEINE eigene Logik.

    Bis T-0177 stand hier eine ausformulierte Zweitimplementierung mit dem
    Kommentar "seit dem T-0097-Fix identisch im Kern". Genau diese Behauptung
    war das Problem: sie stimmte, aber nichts hielt sie fest. Gemessen am
    23.08.26 lief ein Drift, der nur `third_place` traf, durch die komplette
    Suite (629/629 gruen) -- `test_rival_profiles` pinnt nur `group` und einen
    K.o.-Elferfall mit festen Zahlen.

    Jetzt gibt es die Regel einmal (`scoring.kicktipp_points`), und
    `test_scoring_drift_guard.py` haelt zusaetzlich fest, dass dieser Zweig
    nicht wieder auseinanderlaeuft. Der Adapter bleibt, weil die Aufrufer hier
    mit Tupeln statt `Score` arbeiten.
    """
    return kicktipp_points(Score(*pred), Score(*act), stage, round_id=round_id)
...
def _stage_of(predictions_by_id, mid: str) -> str | None:
    """Stage der Partie, oder None wenn sie nicht bekannt ist (fehlende
    Prediction oder fehlendes fixture.stage).

    T-0179: Frueher stand hier ein stiller Default auf "group" -- ein K.o.-
    Spiel ohne Prediction wurde dadurch unbemerkt mit Gruppen-Punkten gewertet.
    Ein fehlender Wert bedeutet jetzt "unbekannt", nicht "Gruppe"; der Aufrufer
    muss das Match dann als ungewertet behandeln statt zu raten."""
    p = predictions_by_id.get(mid) or {}
    return (p.get("fixture") or {}).get("stage")
...
def _profile(name, tips, actuals, model_tips, predictions_by_id, round_id) -> dict[str, Any] | None:
    ...
    sim = sim_n = pts = exact = played = skipped_stage = 0
    for mid, (th, ta) in parsed.items():
        ...
        if act is not None:
            ...
            if stage is None:
                # T-0179: Stage unbekannt -> nicht werten statt still als
                # "group" zu zaehlen (siehe Kommentar bei _stage_of).
                skipped_stage += 1
                continue
    ...
    return {
        ...
        "played": played,
        "skipped_stage": skipped_stage,  # T-0179: Matches ohne bekannte Stage, ungewertet
        "points": pts,
