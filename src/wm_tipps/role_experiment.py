...
def normalize_manual_results(payload: Any) -> dict[str, dict[str, Any]]:
    ...
    for match_id, value in (results or {}).items():
        if isinstance(value, list) and len(value) == 2:
            ...
            # T-0185: Laenge PRUEFEN, nicht nur auf truthy testen. Vorher war die
            # Funktion inkonsistent -- eine blanke Liste falscher Laenge (`[2]`)
            # wurde still uebersprungen, `{"actual": [2]}` warf dagegen
            # IndexError. Aufrufer, die bisher selbst `len(act) != 2 -> continue`
            # pruefen mussten, koennen sich jetzt auf das Ueberspringen verlassen.
