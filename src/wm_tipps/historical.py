...
# T-0187: Sektion -> echte Turnier-Stage im Vokabular der Fixtures/Rundenregeln.
# Vorher stand hier eine Menge und JEDE KO-Zeile bekam pauschal "knockout" --
# ein Wort, das in KEINER Rundenkonfiguration als Stage vorkommt. Runden mit
# `stage_points` (Eskalation ab Achtelfinale) liefen deshalb im Backtest auf
# ihren Basissatz zurueck: gemessen 170 statt 340 Punkte fuer die Zweitrunde
# ueber 105 KO-Spiele. Singular- und Plural-Varianten, weil EM 2020
# "Semi-final" schreibt.
FINALS_KO_STAGES = {
...
# Rueckwaertskompatibler Alias: frueher exportierte das Modul die Menge.
FINALS_KO_SECTIONS = frozenset(FINALS_KO_STAGES)
...
def parse_openfootball_finals(text: str) -> list[dict[str, Any]]:
    ...
    Jede Zeile traegt die ECHTE Stage ihrer Sektion (round_of_16, quarter,
    semi, final) -- dasselbe Vokabular wie die Fixtures, damit Rundenregeln
    mit Stage-Eskalation im Backtest greifen (T-0187).
    """
    results: list[dict[str, Any]] = []
    ...
    for raw_line in text.splitlines():
        ...
        if section and "|" not in line:
            current_stage = FINALS_KO_STAGES.get(_normalize_section(section.group(1)))
            continue
...
def build_historical_dataset(tournament: str, *, write: bool = True, force_fetch: bool = False) -> dict[str, Any]:
    ...
    # Echte Stages statt des pauschalen "knockout" (T-0187), in Turnier-
    # Reihenfolge statt alphabetisch -- sonst steht "final" vor "quarter".
    stage_order = ["group", *FINALS_KO_STAGES.values()]
    seen_stages = {str(row.get("stage")) for row in results}
    stages_covered = [stage for stage in dict.fromkeys(stage_order) if stage in seen_stages]
