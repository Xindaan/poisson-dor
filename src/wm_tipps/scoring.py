...
# --------------------------------------------------------------------------- #
# Favoriten-relative Tipps + EP-Max-/Chase-Tipp auf einer fertigen Verteilung.
# T-0181: aus analysis/win_sim_v2.py UND src/wm_tipps/deficit_policy.py hierher
# zusammengelegt -- beide hatten dieselbe Logik wortgleich dupliziert, einmal
# mit lokalen String-/Tupel-Parse-Helfern, einmal mit `Score`. Aequivalenz
# vor dem Umbau per tests/test_sim_helpers_equivalence.py belegt. Beide Seiten
# rufen jetzt diese Funktionen; die Aufrufer transformieren die Verteilung
# (ET-/Elfer-Konvention) VORHER selbst -- hier wird nichts mehr transformiert.
# --------------------------------------------------------------------------- #
MAX_GOALS = 6  # Scoreline-Grid 0..6 fuer EP-Max-/Chase-Suche.


def _score_from_label(label: str) -> Score:
    home, away = (int(part) for part in label.split(":"))
    return Score(home, away)


def model_outcome_probs(dist: Mapping[str, float]) -> tuple[float, float, float]:
    """Normierte (Heim-, Remis-, Auswaerts-)Wahrscheinlichkeit aus einer
    Scoreline-Verteilung."""
    ph = pd = pa = 0.0
...
def fav_side(dist: Mapping[str, float]) -> str:
    """Favorit des Spiels laut Modell-Verteilung ('home'/'away')."""
    ph, _, pa = model_outcome_probs(dist)
    return "home" if ph >= pa else "away"


def rel_of(tip: str, fav: str) -> tuple[int, int]:
    """Tipp favoriten-relativ ausgedrueckt: (Tore Favorit, Tore Gegner), auf
    MAX_GOALS gekappt."""
    score = _score_from_label(tip)
    pair = (score.home, score.away) if fav == "home" else (score.away, score.home)
    return (min(pair[0], MAX_GOALS), min(pair[1], MAX_GOALS))


def unrel(rel: tuple[int, int], fav: str) -> str:
    """Kehrfunktion zu rel_of: favoriten-relatives Paar zurueck in Heim:Gast."""
    fav_goals, opp_goals = rel
    return f"{fav_goals}:{opp_goals}" if fav == "home" else f"{opp_goals}:{fav_goals}"


def best_tip_on_dist(
    dist: Mapping[str, float],
    stage: str,
    round_id: str = DEFAULT_ROUND_ID,
) -> str:
    """EP-Max-Tipp direkt auf der (ggf. schon transformierten) Verteilung --
    kein erneuter ET-/Elfer-Transform (der passiert vorher, ausserhalb, ueber
    resolve_extra_time/resolve_knockout_draw_probabilities)."""
    best, best_ep = None, -1.0
    for home in range(MAX_GOALS + 1):
        for away in range(MAX_GOALS + 1):
            ep = expected_points(Score(home, away), dist, stage, round_id=round_id)
            if best is None or ep > best_ep:
                best, best_ep = f"{home}:{away}", ep
...
def chase_tip(
    ...
    """Tipp, der P(schlaegt target_tip) maximiert (Tiebreak: hoeheres EP,
    Epsilon 1e-12 gegen Gleitkomma-Rauschen)."""
    target = _score_from_label(target_tip)
    best, best_pg, best_ep = None, -1.0, -1.0
    for home in range(MAX_GOALS + 1):
        for away in range(MAX_GOALS + 1):
            candidate = Score(home, away)
            pg = ep = 0.0
            for label, p in dist.items():
                actual = _score_from_label(label)
                candidate_points = kicktipp_points(candidate, actual, stage, round_id=round_id)
                if candidate_points > kicktipp_points(target, actual, stage, round_id=round_id):
                    pg += p
                ep += p * candidate_points
            if pg > best_pg + 1e-12 or (abs(pg - best_pg) < 1e-12 and ep > best_ep):
                best, best_pg, best_ep = f"{home}:{away}", pg, ep
    assert best is not None
    return best
