...
def command_row(
    ...
    # T-0176: 'gespielt' ist terminal -- Pre-Match-Checks sind damit erloschen,
    # nicht nur unwichtig. Ohne dieses Leeren zaehlt `command_summary.open_due`
    # sie weiter mit (real gemessen: open_due 104 bei played 104) und
    # `append_rows` rendert sie als offene Checks. Die Rohliste `actions` bleibt
    # bewusst erhalten: sie ist die Historie, `due_actions`/`next_actions` sind
    # die Arbeitsliste. Gleiche Fehlerklasse wie der `played`-Guard aus T-0169 --
    # verfehlt wurde sie dort, weil jener Check nach `status == "played"` suchte
    # und dieses Modul den Terminalzustand aus `kickoff <= now` ableitet
    # (bewusst, wegen des Fixture-Lags aus T-0123). Zwei Vokabulare, ein Zustand.
    # `upcoming_actions` ist heute per Konstruktion schon leer (jeder Check hat
    # ein `due_at` relativ zum Anpfiff); das Leeren ist Defensive, falls je ein
    # Check ohne Faelligkeit dazukommt.
    if status == "gespielt":
