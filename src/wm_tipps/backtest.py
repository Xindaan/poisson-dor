...
def report_paths_for_round(round_id: str = DEFAULT_ROUND_ID) -> tuple[Path, Path]:
    """Report-Dateien je Runde (T-0193).

    Die Default-Runde behaelt ihre bisherigen Pfade -- sonst waeren alle
    Konsumenten (Dashboard, README-Guard, public-Manifest) zu aendern. Jede
    weitere Runde bekommt einen eigenen Satz. Vorher rechnete `backtest-report`
    NUR die Default-Runde; ein Verdict fuer die Zweitrunde stand zwar in
    STATE.md, aber es gab keinen Codepfad, der ihn erzeugt haette.
    """
    if round_id == DEFAULT_ROUND_ID:
        return BACKTEST_REPORT_PATH, BACKTEST_REPORT_MARKDOWN_PATH
    slug = re.sub(r"[^a-z0-9]+", "-", round_id.lower()).strip("-") or "runde"
    return (
        DATA_DIR / f"backtest_report_{slug}.json",
        EXPORTS_DIR / f"backtest_report_{slug}.md",
    )
...
def run_backtest(dataset_path: Path | str | None = None, round_id: str = DEFAULT_ROUND_ID) -> dict[str, Any]:
    ...
    return {
        ...
        # T-0187: ueber is_knockout_stage statt Literal "knockout" -- die
        # Datensaetze tragen seit dem Stage-Fix round_of_16/quarter/semi/final.
        "knockout_matches": sum(1 for r in rows if is_knockout_stage(r.get("stage", "group"))),
        "round_id": round_id,
        "variants": variants,
