...
def _knockout_fixture(
    ...
    *,
    ...
    if existing and not has_pending_slot:
        ...
        # Frueher wurde per match_number uebernommen -- korrigiert jemand ein
        # Gruppenergebnis, nachdem eine K.o.-Runde schon erfasst war, erbt die
        # neue Paarung still das Ergebnis der alten und der Baum laeuft mit
        # einem falschen Sieger weiter. Die Frische-Pruefung sah das nicht: das
        # Fixture stand ja auf "played".
        # Platzhalter ohne Teams (aus `_manual_knockout_result_placeholders`)
        # und "unbekannt"-Zeilen zaehlen als paarungslos -- dort ist Uebernehmen
        # richtig, denn genau so kommt ein manuelles Ergebnis in den Baum.
        previous_home = str(existing.get("home_team") or "")
        previous_away = str(existing.get("away_team") or "")
        pairing_known = bool(
            previous_home and previous_away
            and previous_home != "unbekannt" and previous_away != "unbekannt"
        )
        pairing_changed = pairing_known and (previous_home != home or previous_away != away)
        for key in ("status", "result", "penalty_winner", "notes"):
            if key not in existing:
                continue
            if key == "status" and existing[key] == "slot_pending":
                continue
            if pairing_changed and key in ("status", "result", "penalty_winner"):
                continue
            fixture[key] = existing[key]
        if pairing_changed:
            fixture["replaced_pairing"] = {
                "previous_match": f"{previous_home} - {previous_away}",
                "dropped_result": existing.get("result"),
                "reason": (
                    "Paarung hat sich geaendert; das Ergebnis der alten Paarung "
                    "wurde nicht uebernommen. Ergebnis in manual_results.json pruefen."
                ),
            }
    return fixture
