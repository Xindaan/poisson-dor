...
# --- T-0191: was der lokale Server ausliefern darf ---------------------------
# Vorher lieferte der Handler die ganze PROJECT_ROOT aus -- also auch `.git/`,
# `data/manual_pool_tips.json` (Klarnamen und Tipps der Mitspieler) und
# `src/wm_tipps/rounds_local.py` (die privaten Runden-Slugs). Auf 127.0.0.1
# faellt das nicht auf; mit `--host 0.0.0.0` liegt es im LAN offen.
# Die Liste ist genau das, was index.html und die drei JS-Dateien laden.
...
# Nur Dateitypen, die das Dashboard braucht -- eine Praefix-Freigabe allein
# wuerde sonst alles mitnehmen, was jemand spaeter unter assets/ ablegt.
ALLOWED_SUFFIXES = (".css", ".js", ".json", ".html", ".svg", ".png", ".woff2")
...
def is_allowed_path(path: str) -> bool:
    """Darf dieser URL-Pfad ausgeliefert werden? (T-0191)

    Arbeitet auf dem URL-Pfad, nicht auf dem Dateisystem: `..`-Segmente und
    absolute Pfade fallen durch, weil sie weder in ALLOWED_FILES stehen noch
    mit einem erlaubten Praefix beginnen.
    """
    ...
    if relative in ALLOWED_FILES:
        return True
    return relative.startswith(ALLOWED_PREFIXES) and relative.endswith(ALLOWED_SUFFIXES)
...
class DashboardRequestHandler(SimpleHTTPRequestHandler):
    ...
    def local_origin_ok(self) -> bool:
        """Ein POST darf nur aus der eigenen Seite kommen (T-0191).

        Ohne diese Pruefung genuegte ein `fetch` mit `text/plain` von einer
        beliebigen fremden Seite, um hier Pipeline-Kommandos zu starten --
        ein simple request, den der Browser ohne Preflight durchlaesst.
        Fehlt der Origin-Header ganz (curl, eigene Skripte), ist es kein
        Browser-Cross-Site-Aufruf und bleibt erlaubt.
        """
    ...
    def do_POST(self) -> None:
        ...
        if not self.local_origin_ok():
            self.write_json_response(
                {"ok": False, "error": "Nur von der lokalen Dashboard-Seite aus startbar."},
                status=403,
            )
            return
        try:
    ...
    def do_OPTIONS(self) -> None:
        # Preflight nur fuer lokale Origins beantworten (T-0191). Vorher stand
        # hier eine feste Allow-Origin-Zeile, die jeder Preflight zurueckbekam.
        if not self.local_origin_ok():
    ...
    def read_json_body(self) -> dict[str, Any]:
        # T-0191: echtes application/json verlangen. Das ist der zweite Riegel
        # gegen Cross-Site-POSTs: mit diesem Content-Type erzwingt der Browser
        # einen Preflight, den do_OPTIONS nur fuer lokale Origins beantwortet.
