...
from .backtest import build_backtest_report, build_blend_weight_sweep, report_paths_for_round
...
def default_update_steps(
    ...
    *,
    ...
    if include_backtest_report:
        ...
        # T-0193: dieselbe Ablation fuer jede weitere Rundenkonfiguration.
        # Ohne diesen Schritt gab es fuer die Zweitrunde nie einen Report --
        # ihr Verdict stand nur als Behauptung in STATE.md.
        for extra_round in (rid for rid in ROUND_ORDER if rid != DEFAULT_ROUND_ID):
            json_path, markdown_path = report_paths_for_round(extra_round)
            steps.append(
                (
                    f"backtest-report:{extra_round}",
                    f"Ablation-Report fuer die Runde {round_name(extra_round)}",
                    lambda rid=extra_round, jp=json_path, mp=markdown_path: build_backtest_report(
                        include_sample=False, round_id=rid, json_path=jp, markdown_path=mp
                    ),
                )
            )
        steps.append(
