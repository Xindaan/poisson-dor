...
from .scoring import (
    ...
    # T-0181: favoriten-relative Tipp-Helfer + EP-Max-/Chase-Tipp sind in den
    # Kern gewandert (siehe scoring.py) -- win_sim_v2.py importiert dieselben
    # Funktionen. Alias auf die bisherigen lokalen Namen, damit hier nichts an
    # den vielen Aufrufstellen umbenannt werden muss.
    fav_side as _fav_side,
    rel_of,
    unrel as _unrel,
    best_tip_on_dist as _ep_tip,
    chase_tip as _chase_tip,
)
...
# MAX_GOALS kommt aus scoring.py (T-0181, single source mit den EP-Max-/
# Chase-Helfern unten).
...
# _fav_side/_unrel/_ep_tip/_chase_tip: siehe Import oben (T-0181, Alias auf
# scoring.fav_side/unrel/best_tip_on_dist/chase_tip -- vorher hier wortgleich
# zu analysis/win_sim_v2.py dupliziert). Nur _rel_of bleibt als duenner
# Adapter lokal: die Aufrufer unten reichen den Tipp schon als geparstes
# Tupel durch (siehe _rival_modal_rel_with_n/_rival_rel_dist), scoring.rel_of
# erwartet dagegen ein Label -- das ist der einzige echte Signaturunterschied,
# den es vor dem Umbau gab.
def _rel_of(tip, fav):
    h, a = tip
    return rel_of(f"{h}:{a}", fav)
...
def build_deficit_policy(*, predictions=None, pool_tips=None, standings=None, fixtures=None, write: bool = True) -> dict[str, Any]:
    ...
    for p in predictions:
        ...
        if xg.get("home") is None:
            continue
        mid = p.get("match_id")
        # T-0184 (Klasse aus T-0179): kein stiller Default auf "group". Die
        # Prediction ist hier zwar garantiert da (siehe `continue` oben), ihre
        # Stage aber nicht -- und ein fehlendes `fixture.stage` als Gruppenspiel
        # zu werten verschleiert einen Datendefekt, statt ihn zu zeigen.
        stage = (p.get("fixture") or {}).get("stage")
        if stage is None:
            continue
