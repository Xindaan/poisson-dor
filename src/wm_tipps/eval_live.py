...
from .results import played_results, results_from_fixtures
...
def _results_from_fixtures(fixtures: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    T-0188: duenner Adapter auf `results.results_from_fixtures`. Die fruehere
    lokale Kopie setzte ``penalty_winner`` hart auf ``None`` und warf damit den
    Elfmeter-Sieger weg, den die Fixture-Zeile traegt -- eine Runde mit
    Elfmeter-Scope wertete ein 1:1 n.V. dann als Remis.
    """
    return results_from_fixtures(fixtures)
...
def build_live_eval(
    *,
    ...
    if snapshots is None:
        snapshots = {}
    if results is None:
        # T-0081/T-0188: Fixtures sind die Basis (status=played + result,
        # stabile match_id), manual_results.json ueberschreibt feldweise
        # (Korrekturen, Elferbilanz). Die Rangfolge steht in `results.py`.
